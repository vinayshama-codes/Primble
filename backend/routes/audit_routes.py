...
async def resolve_issue(
    ...
    endpoints are untouched.
      ...
      mode=add_form  -> generate a missing ACORD form into this package (UX-05)

    `add_form` shares this endpoint on purpose. It needs the identical tail -
    recalculate, re-read, regroup, return `updated_forms` /
    `grouped_cross_issues` / the form-selection view - and the panel's
    `_applyCrossIssuePanelUpdate` already consumes exactly that shape. A second
    ...
    _log_field: str = ""
    _log_value: str = ""
    # UX-05: the summary of a form generated by mode=add_form, so the panel can
    # insert the row without a reload. Empty for every other mode.
    _added_form: dict = {}
    ...
    if mode == "field":
        ...
        # UX-05. The producer clicked "Add ACORD 186" on a validation that says
        # ACORD 186 is missing.
        #
        # THE SERVER DECIDES, NEVER THE WIRE (the BUG-05 contract). The wire
        # carries only the form id; `add_form_to_session` re-derives from the
        # STORED issues whether any open validation is actually asking for that
        # form, re-checks that it is not already in the package, and re-checks
        # that a template exists. A stale tab, a replayed request or a hand-
        # rolled POST therefore cannot create a form nothing asked for - which
        # is the whole of the owner's 2026-09-08 ruling, made executable.
        #
        ...
        from services.form_addition import (
            OK as _ADD_OK, add_form_to_session, message_for as _add_message,
        )
        _new_form_id = (req.add_form_id or "").strip()
        if not _new_form_id:
            return JSONResponse({"success": False, "message": "No form to add."})
        try:
            _outcome, _payload = await add_form_to_session(
                req.session_id, _new_form_id)
        except Exception as _afx:                                  # noqa: BLE001
            logger.error(
                f"resolve_issue: add_form failed [session={req.session_id} "
                f"form={_new_form_id}]: {_afx}", exc_info=True)
            return JSONResponse({
                "success": False,
                "message": "The form could not be generated. Please try again.",
            })
        if _outcome != _ADD_OK:
            return JSONResponse({
                "success": False,
                "outcome": _outcome,
                "message": _add_message(_outcome),
            })
        applied = True
        _added_form = _payload.get("form") or {}
        # Audited like any other producer action. `field_name` is the pseudo-key
        # the schedule path already established for a non-field change, so the
        # export has one shape for "the producer changed something structural".
        try:
            await log_field_change(
                session_id=req.session_id, user_id=str(current_user["id"]),
                form_id=_new_form_id, field_name=f"form::{_new_form_id}",
                fact_key=None, source="producer", record_unchanged=True,
                previous_value="not in package", new_value="added",
                confidence="filled", model_version=SQS_MODEL_VERSION,
            )
        except Exception as _le:
            logger.warning(f"resolve_issue: add_form audit log failed: {_le}")
    ...
    return JSONResponse({
        ...
        # UX-05: present only on mode=add_form. `added_form` is the same summary
        # shape select_forms_bulk returns per form, so the editor can splice the
        # new row into its list; `form_ids` is the package's full, ordered set
        # AFTER the add, because the panel must never rebuild that list from a
        # single-form response (that is exactly how reusing select_forms_bulk
        # would have wiped the other six).
        "added_form":               _added_form or None,
        "form_ids":                 (list((sess.get("selected_form_ids") or []))
                                     if _added_form else None),
...
async def reopen_issue(
    ...
    if not cleared_any:
        ...
        #
        # But SAY WHETHER THE VALIDATION IS ACTUALLY BACK (2026-09-09). The
        # panel renders a resolved-and-no-longer-firing issue as a "ghost" row
        # in the Resolved section, and a live one in its severity section. An
        # issue that is neither - marked open, but not emitted by any rule -
        # renders NOWHERE, so the row silently disappeared and the producer got
        # no answer at all.
        #
        # Found on the UX-05 add-form rows, where it is unavoidable rather than
        # incidental: adding ACORD 25 permanently clears
        # `certificate_requested_but_acord25_missing`, so reopening can never
        # bring it back. The same hole was already there for every schedule /
        # narrative / none-mode issue whose condition had since been fixed
        # elsewhere; this closes the class, not the one case.
        _still_live = False
        if req.issue_id:
            try:
                _sess = await get_processing_session(req.session_id)
                _still_live = any(
                    isinstance(i, dict) and i.get("issue_id") == req.issue_id
                    for i in (_sess.get("cross_issues_last") or [])
                )
            except Exception as _lx:                              # noqa: BLE001
                logger.warning(f"reopen_issue: live-check failed: {_lx}")
        return JSONResponse({
