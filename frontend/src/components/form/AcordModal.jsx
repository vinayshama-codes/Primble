...
export default function AcordModal({
    ...
    fetch(`${API_BASE}/api/session/${resumeSessionId}`, { headers: { Authorization: `Bearer ${token}` } })
      ...
      .then((data) => {
        if (data && data.generated_forms && Object.keys(data.generated_forms).length > 0) {
          ...
          const readyMap = {};
          Object.keys(data.generated_forms).forEach((fid) => { readyMap[fid] = false; });
  ...
  useEffect(() => {
    ...
    const drop  = (e) => {
      ...
      const uploaded = Array.from(e.dataTransfer.files).filter(
        (f) => f.type === "application/pdf" || f.name.toLowerCase().endsWith(".zip") || f.type.startsWith("image/")
      );
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.status === 403) {
        ...
        if (msg.includes("suspended"))    setError("🚫 Your account is suspended. Please restore your billing.");
        else if (msg.includes("archived")) setError("🗄️ Account archived. Contact support@acordly.ai.");
        else if (msg.includes("soft_locked") || msg.includes("locked")) setError("🔒 Account Disabled — please update your billing.");
        else setError(msg);
        return;
      }
      if (!data.success) {
  ...
  const handleGenerateAll = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.status === 403) { setError("🔒 " + (data.detail || data.message || "Access blocked.") + " Please update your billing."); return; }
        ...
        <div className="modal-inner">
          ...
          {inOverage && (
            <div style={{ background: "#fff7ed", border: "1px solid #fed7aa", borderRadius: 8, padding: "9px 14px", fontSize: 12, color: "#92400e", marginBottom: 8, display: "flex", alignItems: "center", gap: 8 }}>
              📋 <span>You're in overage territory — each additional download will be billed at your plan rate on your next invoice.</span>
            </div>
          )}
          ...
          {user && user.subscription_tier !== "free" && (() => {
            ...
            if (ps === "failed")      return (<div className="payment-status-banner payment-status-failed">⚠️ Payment overdue — please{" "}<button onClick={onOpenBillingPortal} disabled={billingPortalLoading} style={{ color: "inherit", fontWeight: 700, textDecoration: "underline", background: "none", border: "none", cursor: billingPortalLoading ? "wait" : "pointer", padding: 0, display: "inline-flex", alignItems: "center", gap: 4 }}>{billingPortalLoading && <BillingBtnSpinner />}update your billing</button></div>);
            return null;
          })()}
            ...
            <div className="editor-layout">
              <div className="editor-sidebar" style={{ overflowY: "auto" }}>
                      ...
                      return (
                        <div key={fid} className={`form-nav-item ${activeFormId === fid ? "form-nav-active" : ""}`} onClick={() => setActiveFormId(fid)}>
                          <div className="form-nav-name">
                            ...
                            {pdfLoading[fid] ? <span className="form-nav-loading"> ⏳</span> : <span className="form-nav-ready"> ✓</span>}
                          </div>
                          {sq && (
                ...
                <div className="download-actions" style={{ paddingBottom: 16 }}>
                  ...
                  {formIdList.length > 1 && (
                    <button className="btn btn-modal-secondary btn-block" onClick={handleDownloadAll}>📦 Download All Forms ({formIdList.length} forms)</button>
                  )}
              ...
              <button className="btn btn-modal-primary btn-block" onClick={handleAcordConfirm} disabled={!acordLicenseChecked || acordModalLoading}>
                {acordModalLoading ? (
                  <span style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
                    <span style={{ width: 14, height: 14, border: "2px solid currentColor", borderTopColor: "transparent", borderRadius: "50%", display: "inline-block", animation: "spin 0.7s linear infinite" }} />Confirming...
                  </span>
