...
✅ Overage downloads allowed past soft buffer — billed automatically next invoice cycle (no hard block)
✅ Payment failure lifecycle: webhook-driven state machine (day 1/7/10/21/60) — all days send email
...
✅ Cover page: AI narrative summary + truly hidden A2A JSON block (font-size 0, invisible), prepended to ZIP bundle
✅ Producer signature: draw/upload, stored as base64 PNG, injected into PDF signature fields on demand
✅ FIXED: Signature injection now detects /FT /Sig typed fields + correct pikepdf indirect object handling
"""
...
app = FastAPI(title="Acordly API", version="12.1.0")
app.add_middleware(
    CORSMiddleware,
...
def _send_generic_email(to_email: str, subject: str, body_txt: str, body_html: str) -> bool:
    ...
    if provider == "resend":
        ...
        try:
            ...
            logger.error(f"SMTP timeout sending to {to_email}"); return False
        except ValueError as ex:
            logger.error(f"SMTP config error: {ex}"); return False
        except Exception as ex:
            logger.error(f"SMTP failed to {to_email}: {ex}")
            try:
                import smtplib, concurrent.futures
                from email.mime.text import MIMEText
                def _do_plain():
                    host = os.getenv("SMTP_HOST","smtp.gmail.com")
                    port = int(os.getenv("SMTP_PORT","587"))
                    user = os.getenv("SMTP_USER",""); pw = os.getenv("SMTP_PASS","")
                    msg2 = MIMEText(body_txt, "plain")
                    msg2["Subject"] = subject; msg2["From"] = from_addr; msg2["To"] = to_email
                    with smtplib.SMTP(host, port, timeout=30) as srv:
                        srv.ehlo(); srv.starttls(); srv.ehlo()
                        srv.login(user, pw)
                        srv.sendmail(from_addr, [to_email], msg2.as_string())
                    return True
                with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                    future = pool.submit(_do_plain)
                    future.result(timeout=45)
                logger.info(f"SMTP: plain-text fallback sent to {to_email}")
                return True
            except Exception as ex2:
                logger.error(f"SMTP plain-text fallback also failed: {ex2}"); return False
    else:
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        if metadata.get('type') == 'overage':
            ...
            if uid and qty > 0 and sid:
                conn = get_db(); cur = conn.cursor()
                cur.execute("""CREATE TABLE IF NOT EXISTS applied_overage_sessions (
                    stripe_session_id TEXT PRIMARY KEY, user_id TEXT NOT NULL,
                    qty INTEGER NOT NULL, applied_at TEXT NOT NULL)""")
                conn.commit()
        ...
        if sub_id:
            ...
            cur.execute("""UPDATE users SET packages_used=0, billing_period_start=%s,
                payment_status='ok', payment_failed_at=NULL WHERE stripe_subscription_id=%s""", (now, sub_id))
            conn.commit(); cur.close(); conn.close()
        ...
        if sub_id:
            ...
            cur.execute("""UPDATE users SET payment_status='failed',
                payment_failed_at=COALESCE(payment_failed_at, %s) WHERE stripe_subscription_id=%s""", (now, sub_id))
            conn.commit()
        ...
        if sub_id and status in ('active', 'trialing'):
            conn = get_db(); cur = conn.cursor()
            cur.execute("UPDATE users SET payment_status='ok', payment_failed_at=NULL WHERE stripe_subscription_id=%s", (sub_id,))
            conn.commit(); cur.close(); conn.close()
        ...
        if sub_id:
            ...
            cur.execute("""UPDATE users SET subscription_tier='free', packages_limit=0, packages_used=0,
                payment_status='ok', payment_failed_at=NULL WHERE stripe_subscription_id=%s""", (sub_id,))
            conn.commit(); cur.close(); conn.close()
...
def _send_payment_failed_email(email: str, name: str, day: int):
    ...
    if day == 1:
        ...
        subject  = "Your Acordly account data has been archived"
        body_txt = (f"Hi {name or 'there'},\n\nYour Acordly account has been archived after 60 days of non-payment. "
                    f"Your data is retained for 90 days. To restore access, update your billing: {billing_portal}\n\nThe Acordly Team")
        body_html = f"""<div style="font-family:Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;">
          <h2 style="color:#475569;">🗄️ Account archived</h2>
          <p>Hi {name or 'there'},</p>
          <p>Your Acordly account has been archived after 60 days of non-payment.</p>
          <p><a href="{billing_portal}" style="background:#e6007a;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600;">Update Billing</a></p>
          <hr style="border:none;border-top:1px solid #e2e8f0;margin:24px 0;">
          <p style="color:#94a3b8;font-size:11px;">Acordly — AI-powered ACORD form automation</p>
        </div>"""
...
async def run_payment_lifecycle():
    ...
    for row in rows:
        try:
            ...
            if hasattr(failed_at, 'tzinfo') and failed_at.tzinfo is None:
                failed_at  = failed_at.replace(tzinfo=timezone.utc)
            elif isinstance(failed_at, str):
...
async def test_email(request: Request):
    ...
    logger.info(f"Test email day={day} to {to}: {'sent ✅' if sent else 'FAILED ❌'}")
...
async def simulate_payment_failure(
    ...
    conn = get_db(); cur = conn.cursor()
    cur.execute(
        "UPDATE users SET payment_status='failed', payment_failed_at=%s WHERE id=%s",
        (fake_dt, current_user['id'])
    )
    conn.commit(); cur.close(); conn.close()
    logger.info(f"DEV: simulated payment failure {days_ago} days ago for user={current_user['id']}")

    email_day = None
    if days_ago >= 21:   email_day = 21
    elif days_ago >= 10: email_day = 10
    elif days_ago >= 7:  email_day = 7
    else:                email_day = 1
    ...
    if new_status != 'failed':
        conn2 = get_db(); cur2 = conn2.cursor()
        cur2.execute("UPDATE users SET payment_status=%s WHERE id=%s", (new_status, current_user['id']))
        conn2.commit(); cur2.close(); conn2.close()

    return {
        "success": True, "payment_failed_at": fake_dt, "days_ago": days_ago,
        "status_set": new_status if days_ago >= 10 else 'failed',
        "email_sent": sent, "email_day": email_day,
    }
    ...
    try:
        ...
        for customer in customers.data:
            ...
            if subs.data:
                ...
                conn = get_db(); cur = conn.cursor()
                cur.execute("""UPDATE users SET subscription_tier=%s, stripe_subscription_id=%s,
                    packages_limit=%s, billing_cycle=%s, billing_period_start=%s, overage_rate=%s,
...
async def create_overage_checkout(req: OverageCheckoutRequest, current_user: dict = Depends(get_current_user)):
    ...
    try:
        session = stripe.checkout.Session.create(
            ...
            success_url=f"{os.getenv('FRONTEND_URL', 'http://localhost:5173')}/?overage_paid=true&qty={qty}&user_id={current_user['id']}&stripe_session_id={{CHECKOUT_SESSION_ID}}",
...
@app.get("/")
def home():
    return {"message": "Acordly API v12.1", "status": "operational"}
...
async def get_pdf(session_id: str, form_id: str,
    ...
    if r.get("signature_applied") and r.get("pdf_bytes"):
        ...
        logger.info(f"get-pdf: serving signed PDF for {form_id} ({len(pdf_bytes)} bytes)")
        return Response(
            content=pdf_bytes,
...
async def update_pdf(req: PDFUpdateRequest, current_user: dict = Depends(get_current_user)):
    ...
    # If user explicitly cleared any sig fields, treat whole form as unsigned
    # so the stamp is NOT re-injected over their typed value
    ...
    if os.path.exists(tpl):
        # Always regenerate from field_state so edits are baked in
        new_pdf_bytes = fill_pdf(tpl, current_state, confidence)

        if was_signed:
            # Re-inject signature so stamp persists after edit
            conn = get_db(); cur = conn.cursor()
            ...
            if sig:
                ...
                for fn in list(field_data_for_sig.keys()):
                    # Blank sig fields before re-injection so stamp renders cleanly
                    # Skip fields the user explicitly cleared — keep their typed value
                ...
                try:
                    ...
                    logger.info(f"update_pdf: re-injected signature for {form_id} after edit")
                except Exception as ex:
...
async def download_pdf(session_id: str, form_id: str, request: Request,
    ...
    if fresh.get('payment_status') == 'suspended':
        return JSONResponse({"success": False, "payment_locked": True,
                             "message": "Account suspended due to non-payment. Please restore your billing to continue."}, status_code=403)
    if fresh.get('payment_status') == 'soft_locked':
        return JSONResponse({"success": False, "payment_locked": True,
                             "message": "Account disabled — please update your billing to restore access."}, status_code=403)
    ...
    if sub in ('essentials', 'professional') and pkgs_limit == 0:
        conn2 = get_db(); cur2 = conn2.cursor()
        cur2.execute("UPDATE users SET packages_limit = %s WHERE id = %s",
                     (100 if sub == 'essentials' else 400, fresh['id']))
        conn2.commit(); cur2.close(); conn2.close()
    ...
    elif sub in ('essentials', 'professional'):
    ...
    return Response(content=zip_buf.getvalue(), media_type="application/zip",
        headers={"Content-Disposition": f"attachment; filename={form_id}_Package.zip",
...
"ai_block": A machine-readable structured JSON object with: submission_id, generated_at (ISO timestamp),
  ...
  risk_flags (array), acordly_version: "12.1", a2a_schema_version: "1.0"
...
Return ONLY the JSON object. No markdown, no backticks, no extra text."""
    ...
    return {"narrative": f"This ACORD submission package was prepared by {org_name} for applicant {facts.get('applicant_name', 'Unknown')}.",
            ...
            "ai_block": {"agent_name": (user.get("full_name", "") if user else ""), "applicant_name": facts.get("applicant_name"),
                         ...
                         "overall_avg_sqs": avg_sqs, "acordly_version": "12.1", "a2a_schema_version": "1.0"}}
...
async def download_all(session_id: str, request: Request, current_user: dict = Depends(get_current_user)):
    ...
    if sub == 'free':
        ...
        default_limit = 100 if sub == 'essentials' else 400
        if pkgs_limit == 0:
            conn2 = get_db(); cur2 = conn2.cursor()
            cur2.execute("UPDATE users SET packages_limit = %s WHERE id = %s", (default_limit, fresh['id']))
            conn2.commit(); cur2.close(); conn2.close()
            logger.info(f"Backfilled packages_limit={default_limit} for user={fresh['id']} tier={sub}")
        conn = get_db(); cur = conn.cursor()
        cur.execute("UPDATE users SET packages_used = packages_used + 1 WHERE id = %s", (fresh['id'],))
        conn.commit(); cur.close(); conn.close()
    ...
    return Response(content=zip_buf.getvalue(), media_type="application/zip",
        headers={"Content-Disposition": "attachment; filename=ACORD_Package_Acordly.zip", "Cache-Control": "no-cache"})
...
async def send_to_epic(session_id: str, form_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if form_id == "all":
        ...
        epic_payload = {"source": "acordly", "version": "12.1", "export_type": "bulk",
        ...
        r = generated[form_id]
        epic_payload = {"source": "acordly", "version": "12.1", "export_type": "single_form",
                        "timestamp": timestamp, "session_id": session_id,
                        "user_email": current_user.get("email"), "organization": org_name,
...
# ══════════════════════════════════════════════════════════════════
#  PRODUCER SIGNATURE — FIXED
# ══════════════════════════════════════════════════════════════════

# ── FIXED: Added /FT /Sig check as primary detection method ──────
...
_SIGNATURE_FIELD_EXCLUSIONS = [
    ...
    "date_of_sign", "signaturedate", "signature_date",
    "designation",
    "title",
    "printed",
    "print_name",
...
def _is_signature_field(field_name: str, field_type: str = "") -> bool:
    """
    Return True ONLY if the field is an actual ink/image signature field.

    PRIMARY check: /FT /Sig — the PDF standard type for signature fields.
    This is the most reliable indicator; ACORD forms use /Sig typed fields.

    SECONDARY check: field name pattern matching (for non-standard forms
    that use text widgets with signature-like names instead of /Sig type).
    """
    # PRIMARY — PDF standard signature field type
    if field_type and "/Sig" in str(field_type):
        return True

    fn = field_name.lower().replace(" ", "_").replace("-", "_").replace(".", "_")

    # Hard exclusion: any field with "date" in the name is a date field
    if "date" in fn:
        return False
    ...
    # Name must match one of the known signature patterns
    return any(pat in fn for pat in _SIGNATURE_FIELD_PATTERNS)


# ── FIXED: Correct pikepdf indirect object handling throughout ───
def _inject_signature_into_pdf(
    ...
    try:
        for page_idx, page in enumerate(pdf.pages):
            ...
            for annot_ref in annot_list:
                ...
                try:
                    ...
                    logger.info(
                        f"Sig field detected: page={page_idx} "
                        f"name={field_name!r} ft={ft_str!r}"
                    )

                    rect = annot.get("/Rect")
                    if rect is None:
                        logger.warning(f"Sig field {field_name!r} has no /Rect — skipping")
                        annots_to_keep.append(annot_ref)
                        continue
                    ...
                    # ── ISSUE 1 FIX: constrain strictly to the field rect ──
                    # Use the exact field dimensions — no expansion beyond bounds.
                    # Add 2pt inset padding so the signature doesn't touch edges.
                    INSET = 1.0
                    ...
                    # Fit inside field maintaining aspect ratio (never exceed field)
                    ...
                    # Clamp — never exceed field dimensions
                    draw_w = min(draw_w, field_w)
                    draw_h = min(draw_h, field_h)

                    # Center within the inset field rect
                    draw_x = x1 + INSET + (field_w - draw_w) / 2.0
                    draw_y = y1 + INSET + (field_h - draw_h) / 2.0

                    # Render at 3× for crispness (field is small so pixels matter)
                    ...
                    # Build appearance stream — BBox matches draw_w × draw_h exactly
                    ap_ops = (
                        f"q "
                    ...
                    # Stamp annotation sized exactly to draw_w × draw_h
                    ...
                    injected += 1
                    logger.info(
                        f"✅ Sig stamped: page={page_idx} field={field_name!r} "
                        f"rect=({draw_x:.1f},{draw_y:.1f},"
                        f"{draw_x+draw_w:.1f},{draw_y+draw_h:.1f}) "
                        f"draw=({draw_w:.1f}×{draw_h:.1f}) "
                        f"field=({x1:.1f},{y1:.1f},{x2:.1f},{y2:.1f})"
                    )

                except Exception as field_ex:
                    logger.warning(
                        f"Sig field processing error page={page_idx} "
                        f"field={field_name!r}: {field_ex}",
                        exc_info=True,
                    )
                    annots_to_keep.append(annot_ref)
        ...
        logger.info(
            f"Signature injection complete: {injected} field(s) stamped, "
...
async def save_signature(
    ...
    conn = get_db(); cur = conn.cursor()
    cur.execute("UPDATE users SET signature_data = %s WHERE id = %s",
                (sig, current_user['id']))
    conn.commit(); cur.close(); conn.close()
...
async def apply_signature(
    ...
    # Blank text previously entered into signature-type fields before image injection.
    # Prevents AcroForm text rendering from appearing on top of the stamped image
    # (NeedAppearances=True causes viewers to render AcroForm values above page content).
    sig_fields_cleared = []
    ...
    if sig_fields_cleared:
        logger.info(
            f"Cleared {len(sig_fields_cleared)} sig text field(s) before injection: "
            f"{sig_fields_cleared}"
        )

    # Run signature injection
    try:
    ...
    logger.info(
        f"✅ Signature applied: user={current_user['id']} form={form_id} "
        f"session={session_id[:8]} pdf_size={len(signed_pdf)}"
    )
    return {
        "success": True,
        "form_id": form_id,
